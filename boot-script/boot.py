...
def stepMkConfig():
    ...
    with open(os.path.abspath(args.config_dir) + '/sigkey.json') as f:
        a = json.load(f)
...
def stepMakeGenesis():
    ...
    run('cp ../keys/sigkey.json ' + os.path.abspath(args.config_dir))
    run('cp ' + args.contracts_dir + '/genesis.json ' + os.path.abspath(args.config_dir) + "/genesis.json")
    run('cp ' + args.contracts_dir + '/activeacc.json ' + os.path.abspath(args.config_dir) + "/activeacc.json")
...
def stepSetFuncs():
    ...
    setFuncStartBlock('f.votagen',  18)
...
parser.add_argument('--contracts-dir', metavar='', help="Path to contracts directory", default='../config/')
parser.add_argument('--cleos', metavar='', help="Cleos command", default='bin/cleos ')
...
args.contracts_dir = args.contracts_dir
